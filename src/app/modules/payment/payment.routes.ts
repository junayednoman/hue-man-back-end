...
paymentRouters.post(
  "/create-session",
  authVerify(["user"]),
  handleZodValidation(paymentValidationSchema),
  paymentControllers.createPaymentSession
);
paymentRouters.post(
  "/create-portia-payment-session",
  handleZodValidation(portiaPaymentSchema),
  paymentControllers.paymentSessionForPortia
);
paymentRouters.get("/callback", paymentControllers.paymentCallback);
paymentRouters.get(
  "/portia-callback",
  paymentControllers.portiaProPaymentCallback
);
paymentRouters.get(
  "/",
  authVerify(["admin"]),
